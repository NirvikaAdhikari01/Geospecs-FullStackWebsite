<%@page import="Utils.StringUtils"%>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1" pageEncoding="ISO-8859-1"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ include file="AdminHeader.jsp" %>

<%@ page import="java.sql.*" %>
<%  
        Integer adminId = (Integer) userSession.getAttribute("adminId");
        if (adminId == null) {
            response.sendRedirect(request.getContextPath() + "/pages/Index.jsp");
        } %>


<%
    //HttpSession userSession = request.getSession();
    Integer AdminId = (Integer) userSession.getAttribute("adminId");

    // Establish database connection
    String url = (StringUtils.LOCALHOST_URL);
    String Dusername = (StringUtils.LOCALHOST_USERNAME);
    String password = (StringUtils.LOCALHOST_PASSWORD);
    Connection conn = null;
    PreparedStatement pstmt = null;
    ResultSet rs = null;

    try {
        Class.forName(StringUtils.DRIVER_NAME);
        conn = DriverManager.getConnection(url, Dusername, password);

        // Query to fetch admin details based on adminId
        String query = "SELECT adminName, email FROM admin WHERE adminId = ?";
        pstmt = conn.prepareStatement(query);
        pstmt.setInt(1, AdminId);

        rs = pstmt.executeQuery();
        if (rs.next()) {
            String adminName = rs.getString("adminName");
            String adminEmail = rs.getString("email");

            // Set adminName and adminEmail as request attributes
            request.setAttribute("adminName", adminName);
            request.setAttribute("adminEmail", adminEmail);
        }
    } catch (Exception e) {
        e.printStackTrace();
    } finally {
        // Close resources
        try {
            if (rs != null) rs.close();
            if (pstmt != null) pstmt.close();
            if (conn != null) conn.close();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
%>

<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Admin Profile</title>
    <link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/stylesheets/ProfileAdmin.css">
</head>
<body>
    <div class="container">
        <h2>Admin Profile</h2>
        <div class="profile-details">
            <p><strong>Admin Name:</strong> <%= request.getAttribute("adminName") %></p>
            <p><strong>Email:</strong> <%= request.getAttribute("adminEmail") %></p>
            <!-- Add more profile details here as needed -->
        </div>
        <form action="<%=request.getContextPath()%>/pages/EditAdminProfile.jsp">
            <button type="submit">Edit Profile</button>
        </form>
    </div>
</body>
</html>
